...
const MedicinesProvider = ({ children }) => {
  ...
  const MedicineByCategories = useCallback(
    async (type) => {
      let link;
      if (type === "all") {
  ...
  const fetchSales = useCallback(async (month , year) => {
    try {
      const response = await axiosApi.get(baseUrl + `pharmacy/inventory/sales?month=${month}&year=${year}`, {
        headers: {
          Authorization: `Bearer ${JSON.parse(Cookies.get("user")).token}`,
        },
      });
  ...
  const fetchCollages = useCallback(async (month , year) => {
    try {
      const response = await axiosApi.get(baseUrl + `pharmacy/inventory/college?month=${month}&year=${year}`, {
        headers: {
          Authorization: `Bearer ${JSON.parse(Cookies.get("user")).token}`,
        },
      });
