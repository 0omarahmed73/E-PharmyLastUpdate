    ...
    if (response.ok) {
      ...
      if (!response.ok) {
        if (response.status === 401) {
          ...
          toast.error("حدث خطأ ما , الرجاء التأكد من البيانات المدخلة");
        }
      }
    }
                  ...
                  <Table striped hover>
                    <thead>
                      <tr>
                        ...
                        <th className={"showFonts"}>المورد</th>
            ...
            <Form onSubmit={handleUsage}>
                              ...
                              return (
                                <Form.Check key={p2.name + i}>
                                  ...
                                  <Form.Check.Label>
                                    ...
                                    <p className="m-0 p-0 mb-1 fw-bold">{`الكمية المتاحة : ${p2.amount}`}</p>{" "}
                                  </Form.Check.Label>
                                </Form.Check>
                              );
                            })}
              ...
              <div className="btns mt-4 d-flex gap-2 me-auto justify-content-end ">
                <ButtonSubmit
                  disabled={
                    Object.keys(radio).reduce((total, key) => {
                      const value = radio[key].price;
                      const amount = amountOfMedicine[key] || 0; // Use 0 if the key is not found in amountOfMedicines
                      return (
                        total + (value === "" ? 0 : Number(value) * amount)
                      );
                    }, 0) > 350 ||
                    usageLoading ||
                    Object.keys(radio).reduce((total, key) => {
